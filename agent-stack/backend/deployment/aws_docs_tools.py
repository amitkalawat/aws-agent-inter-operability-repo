...
def get_aws_docs_client_and_tools():
    """
    Get both MCP client and tools for AWS Documentation
    Client must remain active throughout agent lifecycle
    
    Returns:
        Tuple of (MCPClient, List[tools]) if available, (None, []) if disabled
    """
    try:
        client = get_aws_docs_mcp_client()
        if client is None:
        ...
        # Don't use context manager here - caller must manage it
        tools = client.list_tools_sync()
        tool_names = [getattr(tool, 'name', 'Unknown') for tool in tools]
        print(f"✅ Loaded MCP client with {len(tools)} tools: {', '.join(tool_names)}")
        return client, tools
            
    except Exception as e:
        print(f"⚠️  Failed to get AWS documentation client and tools: {e}")
        return None, []