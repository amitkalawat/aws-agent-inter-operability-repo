...
class MCPManager:
    ...
    def create_nova_canvas_transport(self):
        """Create Nova Canvas MCP transport"""
        try:
            credentials = secrets_manager.get_mcp_credentials()
            bearer_token = self._get_bearer_token()
            
            # Check if Nova Canvas URL is available
            if 'MCP_NOVA_CANVAS_URL' not in credentials or not credentials['MCP_NOVA_CANVAS_URL']:
                raise Exception("MCP_NOVA_CANVAS_URL not configured in secrets")
            
            nova_canvas_url = credentials['MCP_NOVA_CANVAS_URL']
            headers = {
                "Authorization": f"Bearer {bearer_token}",
                "Accept": "application/json, text/event-stream"  # Required for Nova Canvas SSE format
            }
            
            return streamablehttp_client(nova_canvas_url, headers=headers)
        except Exception as e:
            print(f"❌ Failed to create Nova Canvas MCP transport: {e}")
            raise
    ...
    def create_nova_canvas_client(self) -> MCPClient:
        ...
        try:
            ...
            client = MCPClient(lambda: self.create_nova_canvas_transport())
...
def create_agent_with_memory(payload: dict) -> Tuple[Agent, Any, MCPClient, MCPClient, MCPClient, str]:
    """Create agent instance with memory configuration and all MCP clients"""
    ...
    # Create MCP clients for all services
    aws_docs_client = None
    dataproc_client = None
    nova_canvas_client = None
    ...
    try:
        # Try to create DataProcessing client
        dataproc_client = mcp_manager.create_dataproc_client()
    except Exception as e:
        print(f"⚠️  Could not initialize DataProcessing client: {e}")
    
    try:
        # Try to create Nova Canvas client
        nova_canvas_client = mcp_manager.create_nova_canvas_client()
    except Exception as e:
        print(f"⚠️  Could not initialize Nova Canvas client: {e}")
    
    if aws_docs_client and dataproc_client and nova_canvas_client:
...
- Generate images using Amazon Nova Canvas (image generation AI service)
- Create visual content with customizable colors and styles
...
Additional Capability: Image Generation with Amazon Nova Canvas
============================================================
I can generate images using Nova Canvas AI model. When you ask me to create, generate, or draw an image, I'll use Nova Canvas to create high-quality visuals:
- Create artistic images, illustrations, and visual content
- Generate images with specific color schemes and styles
- Support for various image types: logos, marketing materials, creative artwork
- Professional quality image generation for business use
...
- "Design a logo..." or "Make a visual..."
- "Show me an illustration..." or "Draw me..."
- "Create artwork..." or "Generate graphics..."
...
- Use bullet points for lists and clear formatting"""
        ...
        return None, memory_hooks, aws_docs_client, dataproc_client, nova_canvas_client, system_prompt
    else:
...
I have access to our previous conversations, so I can maintain context and continuity across our chat sessions."""
        ...
        return agent, memory_hooks, None, None, None, system_prompt
...
def strands_agent_bedrock(payload):
    ...
    agent, memory_hooks, aws_docs_client, dataproc_client, nova_canvas_client, system_prompt = create_agent_with_memory(payload)
    
    try:
        # Simplified approach: Always use all available MCP clients together
        all_tools = [execute_code_with_visualization]  # Always include code interpreter
        
        # Use triple-nested context managers for all three MCP clients
        if aws_docs_client and dataproc_client and nova_canvas_client:
            # All three clients available - ideal production case
            with aws_docs_client:
                with dataproc_client:
                    with nova_canvas_client:
                        ...
                        nova_canvas_tools = nova_canvas_client.list_tools_sync()
                        all_tools = aws_tools + dataproc_tools + nova_canvas_tools + [execute_code_with_visualization]
                        
                        print(f"🛠️  Agent configured with {len(aws_tools)} AWS docs tools")
                        print(f"🛠️  Agent configured with {len(dataproc_tools)} DataProcessing tools")
                        print(f"🛠️  Agent configured with {len(nova_canvas_tools)} Nova Canvas tools")
            ...
            # Fallback: only code interpreter (production expects all MCP services)
            print(f"⚠️  Not all MCP services available - using fallback mode")
...
async def strands_agent_bedrock_streaming(payload):
    ...
    try:
        ...
        if aws_docs_client and dataproc_client and nova_canvas_client:
            ...
            with aws_docs_client:
                with dataproc_client:
                    with nova_canvas_client:
                        ...
                        print(f"🛠️  Streaming agent configured with {len(dataproc_tools)} DataProcessing tools")
                        print(f"🛠️  Streaming agent configured with {len(nova_canvas_tools)} Nova Canvas tools")
                        print(f"🛠️  Streaming agent configured with 1 code interpreter tool")
            ...
            # Fallback: only code interpreter (production expects all MCP services)
            print(f"⚠️  Not all MCP services available - using fallback mode")
            print(f"🛠️  Fallback streaming agent configured with 1 code interpreter tool only")
